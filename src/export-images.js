            ...
            for (let i = 0; i < ops.fnArray.length; i++) {
                try {
                    if (
                        ...
                        const directoryPath = path.join(__dirname, '../' + path.join(path.join(dst, fileName)));

                        mkdirSync(directoryPath, {recursive: true})

                        const file = path.join(directoryPath, `${name}.png`)

                        await sharp(img.data, {
                            raw: {width, height, channels}
                        }).toFile(file)

                        const event = {name, kind, width, height, channels, bytes, file, page: p}
                        images.push(event)
                    }
